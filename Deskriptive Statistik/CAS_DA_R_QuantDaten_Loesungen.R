...
# Ungruppierte Vergleiche
prop.table(table(faithful$waiting))

# Range von Wartezeigen bestimmen
waiting.range <- range(faithful$waiting)

# Gruppenbrueche bestimmen
waiting.breaks <- seq(40, 100, by = 5)

# Effektive Wartezeiten in Gruppen verteilen
waitings.cuts <- cut(faithful$waiting, waiting.breaks, right = FALSE)
...
plot(waiting.breaks, waitings.relfreq.cum0, 
     main = "old faithful eruptions",
     xlab = "cumulative waiting proportion", ylab = "waiting duration in seconds"
)
